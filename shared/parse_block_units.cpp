...
inline int64_t parse_block_units(const std::string &user_input) {
	/***
	inline int64_t parse_block_units(std::string user_input):
	...
	***/
	try {
		...
		if (last_character == "K" || last_character == "k") {
			int64_t blocksize = stol(all_but_last_character);
		...
		*/
		...
		return static_cast<int64_t>(stol(user_input));
	} catch (...) {
		return -1;
	}
}
